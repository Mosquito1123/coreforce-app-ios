...
extension BatteryRenewViewController:UITableViewDataSource,UITableViewDelegate {
    ...
    fileprivate func updateDatas(){
        ...
        let newPackageCard  = BuyPackageCard(title: "费用结算",subtitle: "", identifier: FeeDetailViewCell.cellIdentifier(),packageCard: self.packageCard,boughtPackageCard: self.boughtPackageCard,batteryDetail: self.batteryDetail,coupon: self.coupon,depositService: self.depositService)
        self.updateItem(where: { packageCard in
